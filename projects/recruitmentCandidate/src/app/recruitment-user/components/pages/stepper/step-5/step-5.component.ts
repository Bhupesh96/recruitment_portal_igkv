    ...
    interface Parameter {
      ...
      control_type: 'T' | 'TR' | 'DT' | 'DE' | 'A' | 'DY' | 'DP' | 'D';
...
// ✅ NEW: Custom Validator to check for overlapping dates across all rows in the FormArray
       ...
       * Runs natively under non-secure local environments (HTTP).
       */
      ...
      async submitForm(): Promise<void> {
        ...
        // Mandatory subheading/parameter validation
        if (!this.form.get('mandatorySubheadingsSelected')?.value) {
          const firstMissed =
            this.form.get('firstMissedMandatory')?.value ||
        ...
        this.subheadings.forEach((sub, index) => {
          ...
          if (
            formArray?.controls.some((group) =>
              group.hasError('dateRangeInvalid')
            )
          ) {
            hasDateError = true;
          }
        ...
        try {
          ...
          // ============================================================
          // BUILD DETAIL AND PARAMETER PAYLOAD
          // ============================================================
              ...
              if (
                ...
                for (const param of paramsList) {
                  ...
                  if (isNewFile) {
                    ...
                    const fileHash =
                      await this.calculateFileHash(paramValue);
                  ...
                  // ----------------------------------------------------
                  // Add parameter only if required
                  // ----------------------------------------------------
          ...
          // Important: reject submitForm()
          // so stepper cannot proceed
          throw err;
          ...
          if (formArray) {
            ...
            const processedRows = activeRows.map((row: any, rowIndex: number) => {
              ...
              params.forEach((param) => {
                ...
                if (
                  ['DE', 'DY', 'DP', 'DC', 'DT', 'D'].includes(param.control_type) &&
                  param.isQuery_id
                ) {
