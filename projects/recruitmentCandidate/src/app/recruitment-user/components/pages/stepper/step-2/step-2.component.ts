  ...
  export class Step2Component implements OnInit {
    ...
    private checkMandatorySubheadingsAndParameters(): void {
      ...
      // 1. Identify all 5 degrees from the subheadings array
      const hsscSub = this.subheadings.find(s => s.m_rec_score_field_id === 2);    // 10+2
      const ugSub   = this.subheadings.find(s => s.m_rec_score_field_id === 3);    // UG
      const pgSub   = this.subheadings.find(s => s.m_rec_score_field_id === 4);    // PG
      const phdSub  = this.subheadings.find(s => s.m_rec_score_field_id === 6);    // PHD
      const netSub  = this.subheadings.find(s => s.m_rec_score_field_id === 3080); // NET

      let higherEduMissing = false;

      // LOGIC: Only apply the rule if ALL 5 IDs exist in the API response (current advertisement)
      if (hsscSub && ugSub && pgSub && phdSub && netSub) {

        // Get unique keys for the ones we need to check selection status
        const pgKey  = this.getUniqueKey(pgSub, this.subheadings.indexOf(pgSub));
        const phdKey = this.getUniqueKey(phdSub, this.subheadings.indexOf(phdSub));
        const netKey = this.getUniqueKey(netSub, this.subheadings.indexOf(netSub));

        const pgSelected  = this.form.get(`is${pgKey}Selected`)?.value;
        const phdSelected = this.form.get(`is${phdKey}Selected`)?.value;
        const netSelected = this.form.get(`is${netKey}Selected`)?.value;

        // VALIDATION RULE: If PG is selected, user MUST select at least one: PHD or NET
        if (pgSelected && !phdSelected && !netSelected) {
          higherEduMissing = true;
          firstMissedMandatory = `${phdSub.score_field_title_name} or ${netSub.score_field_title_name} (Compulsory if PG is filled)`;
        }
      }

      // 2. Check for standard mandatory subheadings (where score_field_is_mandatory === '1')
      // We skip this if we already found the Higher Education error to avoid confusing the user
      const missedMandatorySubheading = higherEduMissing ? null : this.subheadings.find(
        (sub, index) =>
      ...
      if (higherEduMissing) {
        // Error message already set in firstMissedMandatory
      } else if (missedMandatorySubheading) {
        firstMissedMandatory = missedMandatorySubheading.score_field_title_name;
      } else {
        // 3. Check for mandatory internal parameters (is_mandatory === 'Y') inside selected sections
        for (let i = 0; i < this.subheadings.length && !firstMissedParameter; i++) {
            ...
            for (const group of formArray.controls) {
              for (const param of params) {
                if (param.is_mandatory === 'Y') {
                  ...
                  const filePath = this.getFilePath(key, param.m_rec_score_field_parameter_new_id, formArray.controls.indexOf(group));
      ...
      // 4. Update the final form status
      const isFormValid = !firstMissedMandatory && !firstMissedParameter;

      this.form.get('mandatorySubheadingsSelected')?.setValue(isFormValid, { emitEvent: false });

      this.form.get('firstMissedMandatory')?.setValue(
        firstMissedMandatory || (firstMissedParameter ? `${firstMissedParameter} in ${firstMissedSubheading}` : ''),
        { emitEvent: false }
      );
    }
