...
fetchStepDisplayOrder() {
    ...
    // ✅ FIX 1: Changed `this.httpService` to `this.http`
...
get activeSectionIds(): number[] {
    ...
    if (this.activeSteps && this.activeSteps.length > 0) {
      validIds = this.activeSteps.map(step => step.compId).filter(id => id >= 2 && id <= 6 && this.formData[id]);
    } else {
      validIds = [2, 3, 4, 5, 6].filter(id => {
  ...
  getProcessedPersonalInfo(): { key: string; value: string }[] {
    ...
    processedData.push({
      ...
      value: this.toTitleCase(fullNameE),
    });
    ...
    processedData.push({
      key: 'Advertisement No',
      value: this.advertisementNo,
    });
    processedData.push({
      key: "Father's Name",
    ...
    processedData.push({
      key: 'Birth Place',
      value: this.toTitleCase(birthPlace),
    });
    ...
    processedData.push({
      ...
      value: this.toTitleCase(permanentAddress),
    });
    ...
    if (info['presentSame']) {
      ...
      processedData.push({
        key: 'Current Address',
        value: this.toTitleCase(currentAddress),
      });
    }
  ...
  getSortedExperienceKeys(stepIndex: number): string[] {
    const stepData = this.formData[stepIndex];
    if (!stepData) return [];

    const keys = Object.keys(stepData).filter(
      (k) => this.isExperienceKey(k) && stepData[k]?.length > 0,
    );
  ...
  getSortedSubheadingKeys(stepIndex: number): string[] {
    const stepData = this.formData[stepIndex];
    ...
    if (value instanceof File) {
      ...
      if (value === 'FILE_UPLOADED' || value === 'File Uploaded') {
        this.alertService.alert(
          true,
          'File preview is not available until the application is fully submitted/saved.',
        );
      } else {
      ...
      this.alertService.alert(
        true,
        'File preview is not available for this local object.',
      );
    }
  }
  ...
  getAppliedCategory(): string {
    ...
    if (
      info['additionalInfoDetails'] &&
      Array.isArray(info['additionalInfoDetails'])
    ) {
    ...
    for (const subheadKey of Object.keys(stepData['subheadings'])) {
      ...
      if (subhead && Array.isArray(subhead.items)) {
        ...
        if (foundItem) {
          ...
          return (
            foundItem.score_field_title_name || foundItem.score_field_name_e
          );
        }
      }
    }
  ...
  makePayment() {
    ...
    if (
      !payload.payee_detail.amount ||
      !payload.payee_detail.receipt_student_data_source_id
    ) {
      this.alertService.alert(
        true,
        'Payment amount could not be loaded. Please refresh the page and try again.',
      );
      return;
    }

    this.receipt_student_data_source_id =
      payload.payee_detail.receipt_student_data_source_id;
    this.alertService.showLoading('Processing Payment', 'Please wait...');

    this.http
      .postData('/fee/post/saveTransactionPayeeDetail', payload, 'academic')
      .subscribe({
        next: (res: any) => {
          ...
          if (data?.order_id && data?.key && data?.amount) {
            ...
            this.alertService.alert(
              true,
              'Payment gateway could not be initiated.',
            );
          }
        },
        error: (err) => {
  ...
  private openRazorpayCheckout(data: any) {
    ...
    rzp.on('payment.failed', (response: any) => {
      ...
      this.http
        .postData('/fee/post/razorpayPaymentFailed', failurePayload, 'academic')
        .subscribe({
          next: (res: any) => console.log('📝 Payment failure logged:', res),
          error: (err) =>
            console.error('⚠️ Failed to log payment failure:', err),
        });
    });
  ...
  private verifyPayment(paymentResponse: any, data: any) {
    ...
    this.http
      .postData('/fee/post/razorPayPaymentVerification', payload, 'academic')
      .subscribe({
        next: (res: any) => {
        ...
        error: (err) => {
          ...
          this.alertService.alert(
            true,
            'Server not reachable for verification.',
          );
        },
      });
  }
