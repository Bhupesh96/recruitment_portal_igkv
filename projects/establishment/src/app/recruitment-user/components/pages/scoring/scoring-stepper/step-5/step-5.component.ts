...
export class Step5Component implements OnInit {
  ...
  private createDataRequest(flag: 'S' | 'C'): Observable<{
    ...
    this.createDataRequest('S')
      ...
      .subscribe({
        next: ({ childrenData, parentData, type }) => {
          // ⭐ Only set existing IDs if we are patching screener ('S') data
          ...
          this.subheadings.forEach((sub, i) => {
            ...
            if (recordsByRow.size === 0) {
              ...
              recordsByRow.forEach((rowItems) => {
                ...
                // ⭐ Only patch database IDs for existing screener ('S') data
                if (type === 'S') {
                  const firstItemOfRow = rowItems[0];

                  // ✅ CORRECTED: ID is now patched for both 'S' and 'C' data.
